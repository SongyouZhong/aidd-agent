...
# ---------------------------------------------------------------------------
_GET_CACHE: dict[str, tuple[float, Any]] = {}  # cache_key → (expires_at, value)
_GET_CACHE_TTL: float = 60.0  # seconds


def _cache_key(url: str, params: dict[str, Any] | None) -> str:
    params_str = json.dumps(params, sort_keys=True, default=str) if params else ""
    raw = f"{url}:{params_str}"
    return hashlib.md5(raw.encode()).hexdigest()  # nosec: not used for crypto
...
async def query_rest_api(
    ...
    *,
    ...
    GET requests with ``expect_json=True`` are cached process-wide for
    ``_GET_CACHE_TTL`` seconds to avoid redundant calls from parallel nodes.
    """
    merged_headers = {"User-Agent": DEFAULT_USER_AGENT, "Accept": "application/json"}
    if headers:
        merged_headers.update(headers)

    # --- TTL cache (GET + JSON only) ---
    cache_hit_key: str | None = None
    if method.upper() == "GET" and expect_json:
        cache_hit_key = _cache_key(url, params)
        entry = _GET_CACHE.get(cache_hit_key)
        if entry is not None:
            expires_at, cached_value = entry
            if time.monotonic() < expires_at:
                return cached_value
            # Expired — remove stale entry
            _GET_CACHE.pop(cache_hit_key, None)
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        for attempt in range(max_retries):
            try:
                ...
                # Store successful GET responses in the TTL cache.
                if cache_hit_key is not None:
                    _GET_CACHE[cache_hit_key] = (time.monotonic() + _GET_CACHE_TTL, result)
                return result
