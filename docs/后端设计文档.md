...
| Method | Path | 说明 | 鉴权 |
|--------|------|------|------|
| POST | `/api/v1/chat` | 发送消息，返回 SSE 流式响应 | ✅ JWT |

> **协议选型**：采用 SSE（Server-Sent Events）而非 WebSocket，与 ChatGPT、Claude、Gemini 三大主流 AI 产品一致。用户消息通过 POST body 发送，服务端通过 `Content-Type: text/event-stream` 流式推送 token。详见第 6 节。
...
> 采用 `POST` + `StreamingResponse` 模式，与 ChatGPT / Claude / Gemini 的实现方式一致。用户消息通过 POST body 发送，Agent 的回复通过 `text/event-stream` 逐 token 推送。

### 6.1 为什么不用 WebSocket

| 维度 | SSE | WebSocket |
|------|-----|-----------|
| **复杂度** | 基于普通 HTTP，FastAPI 原生支持 `StreamingResponse` | 需要连接管理、心跳、重连、状态机 |
| **可靠性** | HTTP/2 多路复用，浏览器自动重连 | 手动实现心跳 + 指数退避重连 |
| **代理兼容** | 通过标准 HTTP，Nginx/CDN 友好 | 需特殊配置 `upgrade` 头 |
| **鉴权** | 直接用 `Authorization` 请求头 | 只能通过 URL query 传 token（不安全） |
| **调试** | 浏览器 DevTools 直接可看 EventStream | 需要专门的 WS 调试工具 |
| **适用场景** | 服务端→客户端单向推送（正是 AI 对话的场景） | 双向通信（游戏、协同编辑） |

AI 对话本质是 Request-Response 模式（只是 Response 是流式的），SSE 完美匹配。
...
async def chat(
    ...
    """发送消息并获取 SSE 流式响应。前端用 fetch + ReadableStream 消费。"""
...
async def stream_chat(
    ...
    """执行 Agent 对话并流式输出 SSE 事件。

    核心流程:
      1. 加载历史消息 (Redis 热缓存 / SeaweedFS 冷存储)
      2. 保存 user message (双写 Redis + S3)
      3. 流式执行 Agent graph（使用 LangGraph astream_events）
      4. 逐事件 yield SSE 格式
    ...
    await append_message(session_id, {
        ...
        "content": user_content, "ts": _now_iso(),
    })
    ...
    async for event in agent.astream_events(
        ...
        if kind == "on_chat_model_stream":
            ...
            if chunk and chunk.content:
                ...
                event_type = "thinking_delta" if _in_thought_block(full_text) else "content_delta"
                yield _sse({"event": event_type, "data": {"delta": chunk.content}})
        ...
        # --- 工具调用结束 ---
            ...
            yield _sse({"event": "tool_use_end", "data": {
                "tool_call_id": event.get("run_id", ""),
                "result_summary": output[:200] + ("..." if len(output) > 200 else ""),
            }})

    # 4. 发送 message_end
    yield _sse({"event": "message_end", "data": {
...
### 6.4 LLM Provider 流式方法 (`stream()`)

当前 `provider.generate()` 是一次性返回，需要增加 `stream()` 方法实现逐 token 输出：

```python
# app/agent/llm_provider.py  —  新增数据类
@dataclass
class StreamChunk:
    type: str              # "text" | "tool_call" | "grounding"
    content: str = ""
    tool_name: str = ""
    tool_args: Any = None
    metadata: Any = None
...
async def stream(self, messages, tools=None) -> AsyncGenerator[StreamChunk, None]:
    ...
    # ... 构建 config（同 generate）

    # 使用 google-genai 原生流式 API
    ...
    for chunk in response_stream:
        for part in chunk.candidates[0].content.parts:
            ...
            if hasattr(part, "function_call") and part.function_call:
                ...
                yield StreamChunk(type="tool_call", tool_name=fc.name,
...
async def stream(self, messages, tools=None) -> AsyncGenerator[StreamChunk, None]:
    ...
    kwargs = {"model": self.model, "messages": oa_messages, "stream": True}  # ← 开启流式
    # ... 构建 tools
...
用户点击「停止生成」时，前端直接 abort 当前 fetch 请求，后端 `StreamingResponse` 的 generator 会收到 `asyncio.CancelledError`，自动清理资源。无需额外端点。

### 6.6 生产环境配置

**Nginx**：SSE 需要禁用缓冲，否则响应会被攒到一起再发：
...
**超时**：Agent 可能执行多轮工具调用，单次对话可能持续 30s+：
